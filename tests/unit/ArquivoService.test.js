...
describe("processMultipleUploads", () => {
  ...
  test("retorna resultados de sucesso para todos os arquivos enviados", async () => {
    ...
    const files = [
      { buffer: jpegBuffer, size: 100, mimetype: "image/jpeg", originalname: "a.jpg" },
      { buffer: jpegBuffer, size: 100, mimetype: "image/jpeg", originalname: "b.jpg" },
...
describe("getById", () => {
  test("gera signed URL fresca quando arquivo está no Supabase", async () => {
    ...
    mockStorageSignedUrl.mockResolvedValue("https://supabase.co/new-signed-url");
...
describe("delete", () => {
  ...
  test("lança ForbiddenError quando usuário não é dono nem admin", async () => {
    ...
    await expect(arquivoService.delete(10, 1, "encarregado")).rejects.toMatchObject({
